...
audio_feats_dir=dataset/tokenized
...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
  ...
  if [ ! -e dataset/manifests/.libritts.done ]; then
    lhotse prepare libritts ${dataset_parts} -j $nj $dl_dir/LibriTTS dataset/manifests
    touch dataset/manifests/.libritts.done
  fi
fi
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
  ...
  if [ ! -e ${audio_feats_dir}/.libritts.tokenize.done ]; then
    python3 tokenizer.py --dataset-parts "${dataset_parts}" \
        --audio-extractor ${audio_extractor} \
        --batch-duration 400 \
        --src-dir "dataset/manifests" \
...
python3 display_manifest_statistics.py --manifest-dir ${audio_feats_dir}
